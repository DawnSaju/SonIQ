            ...
            {records.map((record) => {
              ...
              return (
                <motion.li 
                  ...
                  transition={{ type: "spring", bounce: 0, duration: 0.4 }}
                >
                  <div className="library-scan-row library-scan-row--record">
                    <TrackArtwork candidate={artwork} variant="library" />
                    {editingId === record.id ? (
                      ...
                      <button className="library-scan-copy" type="button" onClick={() => onOpen(record)}><strong>{record.source.fileName}</strong><small>{formatSavedScanDate(record.updatedAt) + " · " + (record.source.durationLabel ?? "Source unavailable")}</small></button>
                    )}
